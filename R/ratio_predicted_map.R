...
ratio_predicted_map <- function(df, sp_key, r1, sdss_legend, r2, cfs_legend, r3, r4, r5, r6, r7, r8, r9) {
  ...
  # C:N ratio goes from 8 up to 351
  # but it's never negative, and if we model it as a regular continuous, it does wonky stuff
  # what if we model NC, which is a proportion, then transform the output?
  df %<>% mutate(NC = CN_ratio^-1)
  
  
  mod <- betareg(NC ~ cfs_lc + sdss_lc + dist_to_coast + ndvi +
...
# apply the model to the stacked rasters
  ...
  # Some of the predicted C:N ratios are into the thousands... can we censor these down to something?
  # Max observed is 350, shall we say 400?
 ...
 g <- ggplot(pred_df) +
  ...
  pred_df %<>%
    rename_with(~paste("CN_", sp_key$species),
                .cols = CN_ratio)
